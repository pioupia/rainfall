...
080484f4 <main>:
 ...
 80484fc:	81 ec a0 00 00 00    	sub    esp,0xa0						; 160
 ...
 8048513:	e8 f8 fe ff ff       	call   8048410 <fopen@plt>			; FILE *file = fopen("/home/user/end/.pass", "r")
 8048518:	89 84 24 9c 00 00 00 	mov    DWORD PTR [esp+0x9c],eax
 804851f:	8d 5c 24 18          	lea    ebx,[esp+0x18]				; char str[132];
 8048523:	b8 00 00 00 00       	mov    eax,0x0
 8048528:	ba 21 00 00 00       	mov    edx,0x21
 804852d:	89 df                	mov    edi,ebx
 804852f:	89 d1                	mov    ecx,edx
 8048531:	f3 ab                	rep stos DWORD PTR es:[edi],eax		; bzero(str, 33);
 8048533:	83 bc 24 9c 00 00 00 	cmp    DWORD PTR [esp+0x9c],0x0
 804853a:	00 
 804853b:	74 06                	je     8048543 <main+0x4f>			; if (!file) return;
 804853d:	83 7d 08 02          	cmp    DWORD PTR [ebp+0x8],0x2		; if (argc != 2) return;
 ...
 804856f:	e8 5c fe ff ff       	call   80483d0 <fread@plt>			; fread(str, 1, 66, file);
 8048574:	c6 44 24 59 00       	mov    BYTE PTR [esp+0x59],0x0		; str[65] = 0
 8048579:	8b 45 0c             	mov    eax,DWORD PTR [ebp+0xc]
 804857c:	83 c0 04             	add    eax,0x4
 804857f:	8b 00                	mov    eax,DWORD PTR [eax]			; argv[1]
 8048581:	89 04 24             	mov    DWORD PTR [esp],eax
 8048584:	e8 a7 fe ff ff       	call   8048430 <atoi@plt>			; atoi(argv[1])
 8048589:	c6 44 04 18 00       	mov    BYTE PTR [esp+eax*1+0x18],0x0	; *(atoi(argv[1])+24) = 0
 804858e:	8d 44 24 18          	lea    eax,[esp+0x18]					; str
 8048592:	8d 50 42             	lea    edx,[eax+0x42]					; str + 66;
 8048595:	8b 84 24 9c 00 00 00 	mov    eax,DWORD PTR [esp+0x9c]			; file
 ...
 80485b3:	e8 18 fe ff ff       	call   80483d0 <fread@plt>				; fread(str + 66, 1, 65, file);
 80485b8:	8b 84 24 9c 00 00 00 	mov    eax,DWORD PTR [esp+0x9c]
 80485bf:	89 04 24             	mov    DWORD PTR [esp],eax
 80485c2:	e8 f9 fd ff ff       	call   80483c0 <fclose@plt>				; fclose(file);
 ...
 80485da:	e8 d1 fd ff ff       	call   80483b0 <strcmp@plt>				; strcmp(str, argv[1]);
 80485df:	85 c0                	test   eax,eax
 80485e1:	75 1e                	jne    8048601 <main+0x10d>				; if (strcmp)
 80485e3:	c7 44 24 08 00 00 00 	mov    DWORD PTR [esp+0x8],0x0
 80485ea:	00 
 80485eb:	c7 44 24 04 07 87 04 	mov    DWORD PTR [esp+0x4],0x8048707	; "sh"
 80485f2:	08 
 80485f3:	c7 04 24 0a 87 04 08 	mov    DWORD PTR [esp],0x804870a		; "/bin/sh"
 80485fa:	e8 21 fe ff ff       	call   8048420 <execl@plt>				; execl("/bin/sh", "sh", 0)
...
Bon ici, on doit juste faire que `strcmp(str, argv[1]);` soit égal à 0, sachant que l'on peut définir un `\0` où l'on veut. Alors soit on se prendre la tête et on essaie de déterminer le mot de passe en brute-force, soit on ruse, et on sait que `argv[1]` vide = `\0` et que `*(atoi(argv[1])+24)` => 24 = 0x18, et que `str` comment à 0x18. Donc si `atoi` renvoie 0, alors `str[0] = 0;`
Bref. String vide:
